...
import omni.usd  # Correct package for USD in Isaac Sim
from omni.usd import Usd, UsdGeom, Gf  
...
def main():
    ...
    set_blue_material(urdf_converter.usd_path)
    # print output
...
def set_blue_material(usd_path):
    stage = Usd.Stage.Open(usd_path)
    if not stage:
        raise ValueError(f"Failed to open USD file: {usd_path}")

    # 找到最后一个 link (根据链接的名字或索引)
    link_name = "box"  # 你可以修改为实际的 link 名称
    link_prim = stage.GetPrimAtPath(f"/airbot_play_v3_0_box/{link_name}")
    ...
    # 创建材质
    material_path = f"/World/{link_name}/Material"
    material = UsdShade.Material.Define(stage, material_path)
    shader = UsdShade.Shader.Define(stage, f"{material_path}/Shader")
    shader.CreateIdAttr("UsdPreviewSurface")

    # 设置颜色为蓝色
    shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(Gf.Vec3f(1.0, 1.0, 1.0))  # 蓝色

    # 将材质赋给 link 的视觉部分
    UsdShade.MaterialBindingAPI(link_prim).Bind(material)
    print(f"Material applied to {link_name}.")
